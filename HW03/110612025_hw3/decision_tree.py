...
class DecisionTree:
    ...
    def _build_tree(self, X: pd.DataFrame, y: np.ndarray, depth: int):
        ...
        if(depth>=self.max_depth or len(np.unique(y))==1):# len(np.unique(y))==1 means all the data in node are in the same class
        ...
        X_left, y_left, X_right, y_right = self._split_data(X, y, feature, threshold)        # Update the progress bar
    ...
    def _best_split(self, X: pd.DataFrame, y: np.ndarray):
        ...
        for feature_index in range(n_features):
            ...
            for threshold in thresholds:
                ...
                if gain > best_gain:
                    best_gain, best_feature_index, best_threshold = gain, feature_index, threshold
