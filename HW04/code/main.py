...
def part5():
    ...
    for i in range(len(epsilons)):
        # Create lists length is 10000 to store the reward and optimal steps in  each steps
        ...
        for time in range(times):
            ...
            # Run the agent in the environment for 10000 steps
            for step in range(steps):
                #Select the action
                action = agent_i.select_action()
                # Take the action in the state to check the reward
        ...
        plt.xlabel("Steps")
        plt.ylabel("Average Rewards")
        plt.title(f"Average Reward With Epsiloms = {epsilons[i]}")

        plt.legend()
        # Plot the optimal action selection percentage
        plt.subplot(1, 2, 2)
        plt.plot(total_step, optimal_action, label="Optimal action percentage")
        plt.xlabel("Steps")
